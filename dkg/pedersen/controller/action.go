...
func (a setupAction) Execute(ctx node.Context) error {
	...
	// Read the threshold flag
	t, err := strconv.Atoi(ctx.Flags.StringSlice("threshold")[0])
	if err != nil {
		return xerrors.Errorf("failed to convert the threshold to int: %v", err)
	}
...
func (a reshareAction) Execute(ctx node.Context) error {
	...
	// Read the thresholdNew flag
	t, err := strconv.Atoi(ctx.Flags.StringSlice("thresholdNew")[0])
	if err != nil {
		return xerrors.Errorf("failed to convert the threshold to int: %v", err)
	}
